...
data1 = np.loadtxt('data/HHG_Opt.txt')
n_max_col = len(data1[0])
col_ini = 1
col_fin = n_max_col
n_slice = 2
label = data1[0,col_ini:col_fin:n_slice]   
a_dipol = data1[1:,col_ini:col_fin:n_slice]   
ncol = len(label)
...
Ip = 0.5-E_true_H2p_R12_asc2_R2 #Ionization potential (a.u.) for H2+ atom at Ground state
...
a_dipol_zero_pad=np.zeros([nTot, ncol], dtype=np.double)
power_spec_HHG=np.zeros([len(harm_HHG), ncol], dtype=np.double)
for i in range(ncol):   
    a_dipol_zero_pad[:nt,i]=windowed(a_dipol[:nt,i],win=win)
    power_spec_HHG[:,i] = np.log10((np.abs(fft(a_dipol_zero_pad[:,i],norm=norm_FFT)[0:nTot//2]))**2) 

# Creating plot
plt.plot(harm_HHG, power_spec_HHG_SSFM, color='C0', linewidth=2, label= label0)  

colors = iter(plt.cm.rainbow(np.linspace(0, 1, ncol)))
for i in range(ncol):
    col = next(colors)
    plt.plot(harm_HHG, power_spec_HHG[:,i], color=col, linewidth=2, label=str(label[i]))
